...
import backend.modules.similarity.StatementGrouper;
...
import backend.modules.report.ReportStore;
    ...
    public AnalysisResponse analyze(CodePayload payload) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = i + 1; j < N; j++) {
                ...
                backend.modules.similarity.ast.ASTNode treeA = backend.modules.similarity.ast.ASTBuilder.build(aComp.rawCode);
                backend.modules.similarity.ast.ASTNode treeB = backend.modules.similarity.ast.ASTBuilder.build(bComp.rawCode);
                backend.modules.similarity.ast.ASTSimilarityResult insensitiveRes = backend.modules.similarity.ast.ASTComparator.compare(treeA, treeB, true);
                
                String verdictLabel = backend.common.util.VerdictUtil.verdict(hyb);
